...
public final class SeedInclusterManifestsUnit extends AbstractManifestsUnit {
  ...
  /**
   * The staged CRD classpath resources — the 2×2 adoption set plus {@code VolumeIntention} (single
   * source: the seed-incluster flake, controller-gen output staged at {@code /crds/} by {@code nix
   * run .#stage-seed-incluster-crd}).
   *
   * <p>{@code VolumeIntention} is not part of the 2×2: it has no Adoption mirror, because that pair
   * exists only where a cluster can be found ALREADY RUNNING and must be reconciled with. A volume
   * is placed or it is not.
   *
   * <p>NOTE: {@code poolreflections} is generated by controller-gen but deliberately absent from
   * this list — check before adding it, the reflector's CRD may be unused in-cluster.
   */
  private static final List<String> CRD_RESOURCES =
      List.of(
          ...
          "/crds/cluster.seedmatic.io_volumeintentions.yaml");
