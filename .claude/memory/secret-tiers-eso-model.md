...
**RENAME APPLIQUÉ (2026-09-10, avant re-grow) : namespace `rke2lab-replicator-source` → `rke2lab-secrets`.** SSOT = nouvelle constante `ClusterRefs.SECRETS_NAMESPACE="rke2lab-secrets"` (à côté de `RUNTIME_SYSTEM_NAMESPACE`), routée dans les 6 unités (Replicator ns-create, FloxController/floxhub-token, Tailscale+TailnetPurge/operator-oauth, FluxReceiver/webhook-token replicate-from) + `.secrets` `kubernetes.sourceNamespace`. Types Java (`ReplicatorSourceSecretsMaterial`/`replicatorSources()`/`ReplicatorSecretsSealScenario`) LAISSÉS (décrivent le fan-out replicator, toujours exact). Décision `rke2lab-system` REJETÉE (le SA cross-cluster ESO scoperait tout le grab-bag système). **RESTE À FAIRE LIVE post-grow : `kubectl delete ns rke2lab-replicator-source` (l'ancien ns devient orphelin — le grow crée le nouveau, ne migre pas l'ancien).** Design d'origine (ci-dessous) : Justif : le namespace n'est plus « la source du replicator » seul — 2 logiques le touchent (ESO in / replicator out) → nommer la charnière neutrement. **PAS `rke2lab-vault`** (« vault » nommerait la source EXTERNE d'ESO, pas la charnière in-cluster ; et sur-vend un HashiCorp Vault qu'on ne déploie pas). Blast-radius : ~5 unités réf. `rke2lab-replicator-source/<secret>` (FloxController/floxhub-token, Tailscale+TailnetPurge/operator-oauth, FluxReceiver/webhook-token, PacSecret), `ReplicatorManifestsUnit` (le namespace L100-113 + annotations mittwald), **`.secrets` `replicateTo` (user, sops) + migration ns live**, couche Java `ReplicatorSourceSecretsMaterial`/`replicatorSources()`/`ReplicatorSecretsSealScenario` (naming seal — renommer aussi ou laisser).
